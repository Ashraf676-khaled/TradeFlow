...
# Run the application
dotnet run --project src/MinimalTradeFlow.Web

# Or run with Aspire (if using)
dotnet run --project src/MinimalTradeFlow.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/MinimalTradeFlow.Web -s src/MinimalTradeFlow.Web
dotnet run --project src/MinimalTradeFlow.Web
```
